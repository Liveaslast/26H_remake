...
void QD4310::SendCommand(const Command cmd, const int16_t value) const {
    ...
    *reinterpret_cast<int16_t *>(TxBuffer + 1) = value;
    ...
    last_tx_timed_out = 0U;
    last_tx_status = static_cast<uint32_t>(HAL_CAN_AddTxMessage(hcan, &TxHeader, TxBuffer, &txMailbox));
    last_tx_error = HAL_CAN_GetError(hcan);
    last_tx_free_level = HAL_CAN_GetTxMailboxesFreeLevel(hcan);
    if (last_tx_status != HAL_OK) {
        return;
    }

    uint32_t guard = 100000U;
    while ((HAL_CAN_GetTxMailboxesFreeLevel(hcan) == 0U) && (guard-- > 0U)) {
    }
    last_tx_free_level = HAL_CAN_GetTxMailboxesFreeLevel(hcan);
    last_tx_error = HAL_CAN_GetError(hcan);
    if (guard == 0U) {
        last_tx_timed_out = 1U;
    }
}
