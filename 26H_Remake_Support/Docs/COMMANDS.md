...
輸出 **Data\Training\Models\best.pt**、訓練記錄及模型幾何資訊。PT 保存在 Windows Support；需要轉換時將它交給本地虛擬機，轉換所得 HEF 經核對後才放入樹莓派運行包 **assets/models/hailo/**。本倉庫不提供 PT→HEF 轉換命令，NCNN 不是正式 Hailo 運行的必要產物。批量改名工具只供預覽，確認配對和備份後才可加 **--apply**：
